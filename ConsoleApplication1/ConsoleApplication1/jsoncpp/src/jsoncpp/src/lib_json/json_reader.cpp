...
Reader::parse( std::wistream& sin,
               ...
               bool collectComments )
{
   //std::wistream_iterator<char> begin(sin);
   //std::wistream_iterator<char> end;
   ...
   std::wstring doc;
   std::getline(sin, doc, (wchar_t)WEOF);
...
Reader::decodeNumber( Token &token )
{
   ...
   if ( isNegative )
      ++current;
#if 0 // [Jonas] convert to number by boost::lexical_cast
   // [Andy] The original logic cannot correct parse the bundary UInt32/UInt64
...
#else
   Value::LargestUInt value;
   std::wstring wstrValue(current, token.end_);
   try {
       value = std::stoull(wstrValue);
   } catch (std::invalid_argument &) {
       return addError( "'" + std::string( token.start_, token.end_ ) + "' is not a number.", token );
   } catch (std::out_of_range &) {
       return addError( "'" + std::string( token.start_, token.end_ ) + "' is overflow.", token );
   }
#endif
   if ( isNegative )
...
Reader::decodeDouble( Token &token )
{
   ...
   if ( length <= bufferSize )
   {
      ...
      memcpy( buffer, token.start_, sizeof(Char) * length );
      buffer[length] = 0;

                      ...
                      unsigned int length = unknown )
{
   ...
   wchar_t *newString = static_cast<wchar_t *>( malloc( sizeof(wchar_t) * (length + 1) ) );
   JSON_ASSERT_MESSAGE( newString != 0, "Failed to allocate string value buffer" );
   memcpy( newString, value, sizeof(wchar_t) * length );
   newString[length] = 0;
   return newString;
}
...
Value &
Value::addEntries( const Value &other )
{
	Json::Value::Members members = other.getMemberNames();
	for(Json::Value::Members::const_iterator i = members.begin(); i != members.end(); ++i) {
		const std::wstring key = *i;
		Json::Value p = other[key];
		if(p.isArray() || p.isObject()) {
			(*this)[key].addEntries(p);
		} else {
			(*this)[key] = p;
		}
	}
                         ...
                         bool isStatic )
{
   ...
   if ( NULL == key || 
       type_ != objectValue )  // [Jonas] expect objectValue, otherwise return null @2009/5/15
       return *this;
...
{
   if ( NULL == key || type_ != objectValue )  // [Jonas] expect objectValue, otherwise return null @2009/5/14
      return null;
   if ( type_ == nullValue )
      return null;
   JSON_ASSERT( type_ == nullValue  ||  type_ == objectValue );
